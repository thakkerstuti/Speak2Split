...
/**
 * Shared account-linking logic for any OAuth provider: if this provider
 * identity is already linked, log in as that user. Otherwise, if an
 * account already exists with the verified email, link the new provider
 * to that EXISTING account (never create a duplicate). Otherwise create a
 * brand-new user + provider link. Every path is keyed off the provider's
 * stable `sub` claim or the verified email — never a display name.
 */
async function linkOrCreateUserForProvider(
  ...
  const created = await UserModel.create({
    // Apple private-relay or missing-email edge case: fall back to a
    // synthetic but stable, unique placeholder tied to the provider sub
    // rather than ever guessing/reusing another account's email.
